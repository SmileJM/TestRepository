...
<script
	...
	function handleBtnSearch() {
		location.href="exam07Search?btitle=${bsearch}";
	}
	</script>
</head>
<body>
	<div style="width: 1000px; margin: auto; text-align: center">
		...
		<a class="glyphicon glyphicon-th-list"  href="exam07List?type=1"></a>
		<a class="glyphicon glyphicon-th-large" href="exam07List?type=2" ></a>
			...
			<c:forEach var="b" items="${list}">
				<tr>
					...
					<td style="text-align: left;  border-left: 0px; border-right: 0px">
						<a href="exam07Detail?type=${param.type }&bno=${b.bno}" >
							<img src="exam07Image?bno=${b.bno }" width="70px" height="70px"/> &nbsp;
							${b.btitle}				
...
<!-- 		<div  class="input-group"  style="margin-top: 20px; width: 1000px">
			...
			<a type="button"  class="btn btn-info"  href="exam07Search?bsearch=김">검색</a>
			<input type="button"  class="btn btn-info" value="검색 " href="exam07List"  onclick="handleBtnSearch()" />
		</div> -->
		<div  class="input-group"  style="margin-top: 20px; width: 1000px">
			<a href="exam07List?type=1&pageNo=1">[처음]</a>
			<c:if test="${groupNo>1}">
				<a href="exam07List?type=1&pageNo=${startPageNo-1}">[이전]</a>
			</c:if>
			<c:forEach var="i" begin="${startPageNo}" end="${endPageNo}">			
				&nbsp;
				<a href="exam07List?type=1&pageNo=${i}"
					<c:if test="${pageNo==i}">style="font-weight: bold; color: red"</c:if>>${i}</a>
				&nbsp;
			</c:forEach>

			<c:if test="${groupNo<totalGroupNo}">
				<a href="exam07List?type=1&pageNo=${endPageNo+1}">[다음]</a>
			</c:if>
			<a href="exam07List?type=1&pageNo=${totalPageNo}">[맨끝]</a>
		</div>
		<div style="margin-top: 10px; width: 700; text-align: right">
			<a href="exam07Write?type=${param.type }" class="btn btn-success">글쓰기</a>
		</div>
		</c:if>
		...
		<div style="width: 1000px; text-align: left; border: 0px;">
		 	<c:forEach var="b" items="${list}" >
				<div style="width: 200px;  height: 330px; float: left;">
					<a href="exam07Detail?type=${param.type }&bno=${b.bno}">
						<img src="exam07Image?bno=${b.bno }" width="180px" height="200px" style="margin-bottom: 10px;"/></a>	
		...
		<div  class="input-group"  style="margin-top: 20px; width: 1000px">
			<a href="exam07List?type=2&pageNo=1">[처음]</a>
			<c:if test="${groupNo>1}">
				<a href="exam07List?type=2&pageNo=${startPageNo-1}">[이전]</a>
			</c:if>
			<c:forEach var="i" begin="${startPageNo}" end="${endPageNo}">			
				&nbsp;
				<a href="exam07List?type=2&pageNo=${i}"
					<c:if test="${pageNo==i}">style="font-weight: bold; color: red"</c:if>>${i}</a>
				&nbsp;
			</c:forEach>

			<c:if test="${groupNo<totalGroupNo}">
				<a href="exam07List?type=2&pageNo=${endPageNo+1}">[다음]</a>
			</c:if>
			<a href="exam07List?type=2&pageNo=${totalPageNo}">[맨끝]</a>
		</div>
		<div style="margin-top: 10px; width: 700; text-align: right">
			<a href="exam07Write?type=${param.type }" class="btn btn-success">글쓰기</a>
		</div>
		...
		<%-- <table class="table table-bordered table-hover" 
			...
			<c:if test="${status.index%5 == 0}"><tr></c:if>
					...
					<div style="width: 200px;">
						<a href="exam07Detail?bno=${b.bno}">
							<img src="exam07Image?bno=${b.bno }" width="200px" height="200px"/>
